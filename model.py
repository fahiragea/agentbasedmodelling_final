...
class AdaptationModel(Model):
    ...
    def __init__(self, 
        ...
        for (u, v) in self.G.edges():
            self.G[u][v]['frequency'] = random.randint(1,10)
    ...
    def step(self):
        ...
        #every 5 (20 ticks) years a small flood, every 20 (80 ticks) years a storm with big flood.
        if self.schedule.steps != 0 and self.schedule.steps % 20 == 0 and self.schedule.steps != 80:
            for agent in self.schedule.agents:
                if agent.is_adapted != True:
                    # Calculate the actual flood depth as a random number between 0.5 and 1.2 times the estimated flood depth
                    agent.flood_depth_actual = random.uniform(0.1, 0.3) * agent.flood_depth_estimated
                    # calculate the actual flood damage given the actual flood depth
                    agent.flood_damage_actual = calculate_basic_flood_damage(agent.flood_depth_actual)
                    agent.savings -= agent.flood_damage_actual * agent.real_estate_value *0.2
                else:
                    agent.flood_depth_actual = random.uniform(0.1, 0.3) * agent.flood_depth_estimated - 1
                    # calculate the actual flood damage given the actual flood depth
                    agent.flood_damage_actual = calculate_basic_flood_damage(agent.flood_depth_actual)
                    agent.flood_opinion += 0.3 * agent.flood_damage_actual
                    agent.savings -= agent.flood_damage_actual * agent.real_estate_value * 0.2
        if self.schedule.steps == 80:
            for agent in self.schedule.agents:
                if agent.is_adapted != True:
                    # Calculate the actual flood depth as a random number between 0.5 and 1.2 times the estimated flood depth
                    ...
                    agent.flood_depth_actual = random.uniform(0.5, 1.2) * agent.flood_depth_estimated - 1
                    # calculate the actual flood damage given the actual flood depth
                    agent.flood_damage_actual = calculate_basic_flood_damage(agent.flood_depth_actual)
                    agent.flood_opinion += 0.3 * agent.flood_damage_actual
                    agent.savings -= agent.flood_damage_actual * agent.real_estate_value * 0.2
